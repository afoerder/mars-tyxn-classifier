#!/usr/bin/env python3
"""
Train a calibrated LinearSVC pipeline on 96x96 Martian patches.
"""
...
def load_split(
    ...
    w = np.asarray([_parse_weight(row, sample_weight_col) for row in split_rows], dtype=np.float64)
...
def build_sample_weight(
    ...
    if m == "none":
        w = np.ones_like(manifest_w, dtype=np.float64)
    elif m == "manifest":
        w = manifest_w.astype(np.float64, copy=False)
    elif m == "non_n_focus":
        w = np.ones_like(manifest_w, dtype=np.float64)
        w[y_str != "N"] = float(non_n_weight)
    elif m == "manifest_non_n":
        w = manifest_w.astype(np.float64, copy=True)
        w[y_str != "N"] *= float(non_n_weight)
    else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train HOG+SVM model on synthetic manifest.")
    ...
    parser.add_argument("--output-model", type=Path, default=Path("models/classifiers/SVM_32.joblib"))
...
def main() -> None:
    ...
    X_val_patch, y_val_str, _, rows_val = apply_task_to_split(
        X_val_patch, y_val_str, np.ones(len(y_val_str), dtype=np.float64), rows_val, args.task
    )
    X_test_patch, y_test_str, _, rows_test = apply_task_to_split(
        X_test_patch, y_test_str, np.ones(len(y_test_str), dtype=np.float64), rows_test, args.task
    )
        ...
        steps=[
            ...
            (
                ...
                CalibratedClassifierCV(
                    estimator=LinearSVC(C=float(args.c), max_iter=10000, tol=1e-4, random_state=args.seed),
                    method="sigmoid",
