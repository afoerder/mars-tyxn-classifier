...
def segmentation_loss(logits, targets, valid, pos_weight, side_logits=None, cldice_weight=0.0):
    ...
    return total, bce, tversky, cldice, ds_loss.detach(), oob_neg.detach()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if DEEP_SUPERVISION:
                ...
                logits = model(images)
                side_logits = []
            loss, bce, tversky, cldice, ds_loss, oob_neg = segmentation_loss(
                logits,
                masks,
                valid,
                batch_pos_weight,
